...
//!
//! 纯数学后处理，不依赖推理引擎，故由本 crate 实现而非 `gtlv-core`。
//! 算法规格见 `gtlv-core` 的 `docs/matching.md`。
...
/// 欧氏距离；维度不等视为不可匹配（+∞）。
///
/// **必须用 f64 累加**：特征是 512 维 f32，按 f32 累加的舍入误差足以选中不同的答案格，
/// 也就是会改变输出结果。规格见 gtlv-core 的 `docs/matching.md`。
